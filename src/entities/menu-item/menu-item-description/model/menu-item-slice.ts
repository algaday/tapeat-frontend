...
export const menuSlice = createSlice({
  ...
  initialState,
  reducers: {
    addModificationRadioGroup: (state, { payload }: PayloadAction<ModificationPayload>) => {
      if (!state.menuItem) {
        return;
      }
    ...
    addModificationCheckboxGroup: (state, { payload }: PayloadAction<Modification>) => {
      if (!state.menuItem) {
        return;
      }
...
export const { addModificationRadioGroup, addModificationCheckboxGroup, updateMenuItem } =
  menuSlice.actions;
