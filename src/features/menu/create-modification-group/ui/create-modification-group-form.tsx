...
export function CreateModificationGroupForm() {
  ...
  const onSubmit: SubmitHandler<CreateModificationGroupSchema> = async (data) => {
          ...
          {fields.map((field, index) => (
            <Stack key={field.id} marginTop={1}>
              <Stack direction={'row'} spacing={1}>
                <RHFInputField name={`modifications.${index}.name`} label="Названия" />
                <RHFInputField name={`modifications.${index}.price`} label="Цена" type="number" />
