...
export function SelectPickup() {
  ...
  const [value, setValue] = useState<LngLat | null>(locationContent.pickupAddress.coordinates);

  const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const modifiedValue = event.target.value.split(',').map((item) => Number(item));
...
function Check(props: { text: string; time: string; value: number[] }) {
  const { updatePickupAddress, updatePickupCoordinates } = useCoordinatesControl();
