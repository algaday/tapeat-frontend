...
export function MarinadeCalculator() {
	...
	const methods = useForm<MarinadeCalculatorSchema>({
		...
		mode: "onSubmit",
		defaultValues: {
			containerWeight: lastSavedContainerWeight
				? parseFloat(lastSavedContainerWeight)
				: 1.4,
		},
	})
	...
	return (
		<Box padding={4}>
			<FormProvider {...methods}>
				<form
					onSubmit={methods.handleSubmit(calculateMarinadeAmount)}
					noValidate
				>
					<Typography
						fontWeight="bold"
					...
					<Divider></Divider>
					<Typography sx={{ color: "#333", mt: 2, fontSize: 17 }}>
						Вес контейнера (кг)
					</Typography>
					<RHFInputField
					...
					{!errorMessage && result && (
						<Alert severity="success">
							<Typography fontSize={19}>
								Чистый вес мясо {result.netMeatWeight}кг.
							</Typography>
							<Typography fontSize={19}>
								Вес маринада {result.marinadeWeight}кг
							</Typography>
						</Alert>
					)}
				</form>
